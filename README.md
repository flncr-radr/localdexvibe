...
Costs and safety: an overlay display puts a small preview window on the phone's own screen, and it writes a device-wide setting. The setting is restored on stop, and the value to restore is now recorded *before* the change, so a session that dies without cleaning up is undone by the next one. If a phantom display is ever left behind anyway, **Developer options → Simulate secondary displays → None** clears it. If the setting can't be written or the display never appears, the session falls back to the virtual display it has always used.
